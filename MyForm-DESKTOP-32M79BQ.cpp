...
System::Void START::MyForm::button3_Click(System::Object^ sender, System::EventArgs^ e)
{
	msclr::interop::marshal_context pre;
	String^ Ключ = textBox1->Text->ToString(); std::string ключ = pre.marshal_as<std::string>(Ключ);   std::vector <bool>  ключ_в_битах = Из16в2(ключ);
	String^ Зашифрованый_текст_в_16_СС = richTextBox2->Text->ToString(); std::string зашифрованый_текст_в_16_СС = pre.marshal_as<std::string>(Зашифрованый_текст_в_16_СС);	//std::vector <char>  текст_в_байтах(текст.begin(), текст.end());
	std::vector <bool> Зашифрованный_текст_в_битах = Из16в2(зашифрованый_текст_в_16_СС);
